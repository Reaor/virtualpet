...
- **嵌入 WebView / 手机**：`pet.js` 在 **粗指针、窄视口、常见移动 UA、`prefers-reduced-motion`** 下自动启用 **`_embeddedMobilePerf`**（略降 DPR 上限、叠分遍数、粒子规模、装饰栅格与暗色斜线底纹）。`new Pet(canvas, { embeddedMobilePerf: true|false })` 可强制覆盖；URL **`?mobilePerf=1`** / **`?mobilePerf=0`** 或 **`?embedded=1`** 传给 `app.js` 中的 `Pet` 构造选项。
