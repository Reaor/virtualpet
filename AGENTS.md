...
- `?dev=1`：除活动区虚线外，在画布**右上角**显示最近一次 **`setForm` 总耗时**、mega 的**定粒/resolve**段、`buildFormLayoutData` 段、粒子数 / `gridCell` / 层级，以及 **`suggestMegaGlyphParticleCount` 缓存条目数**（便于区分「按钮坏了」与「主线程被布局卡住」）。
