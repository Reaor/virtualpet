...
- 巨字呈现辨形与排版启发：仓库保持 **零依赖**，不引入 npm；可参考业界纯 JS 排版思路（例如 [chenglou/pretext](https://github.com/chenglou/pretext) 的测量与折行哲学、Canvas 文本度量）与常见 **mask 内 Poisson / 蓝噪声撒点** 论文/实现，将合适片段以自研函数形式写入 `pet.js`（见 `buildTextSilhouetteLayout` / `suggestMegaGlyphParticleCount` / `getMacroMeasureContext2d` / `segmentStringGraphemes` 等）。
