...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.16.0**：**双运动模态**：**待机**（数学曲线、软团、抽象形态等）保持原幅度与速率；**呈现**（`clock` / `chrono` / `mega` / `kao_*`）统一套用 **display** 内核：`timeScale` 压低有效 `gms`，`ampScale`/`crispMicroScale` 压低流体与体内波，`wanderRadScale`/`wanderPickIntervalMul` 压低游走，`springFollowScale` 缓和整体跟随，`breathMix` 向 1 收敛；华容道/巨字滑步 **冷却加长**。`Pet.motionProfile` 与 `getMotionProfileKernels` / `isDisplayPresentationForm` 对外暴露；侧栏「速」toast 标明当前态。 |
