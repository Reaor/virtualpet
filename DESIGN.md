...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.33.0**：**整块巨字灰轮廓**：**仅「廓」**（`silhouetteMatteUnderlay`）开时 `_drawSilhouetteMatteUnderlay`；**辨** 不再单独触发整块 mask；呈现层 **「廓」默认关**。**巨字排版**：`megaLayoutScale`（侧栏 **字比**，与 `gridCell` 推得的下限取 max）、`macroFitMode`（**容纳**：`shrink` / `truncate` / `wrap2`，多行 `createMacroTextDraw`）；URL **`?megaScale=`**、**`?macroFit=`**。**UI**：左栏 **分区标题 + 双列按钮**、主容器加宽。 |
