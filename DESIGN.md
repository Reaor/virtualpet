...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.17**：**巨字/颜 mask 拖整体贴边**：取消高频 `pulse` 撞散（此前与 `dragTo` 定时触墙叠加，易成「每字乱颤」）；改为 **首次贴上边一次** `edgeEntry` 相干外推 + 缓聚；**非巨字**略放宽脉冲间隔。**沙拨 + 巨字**贴活动区边：**每段贴边只撞散一次**，手指离开边线后可再触发。**3.35.16** 的绘移冲量与外框光晕保留。 |
