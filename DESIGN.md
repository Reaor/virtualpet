...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.8**：**侧栏可读 + 顺滑/有序**：左栏 **188px**、分区 **标题+一句说明**、主要按钮 **双行（主字+副标）**；呈现 mask 剪影 **叠分第二遍隔帧**（关内动不再每帧双遍）、`presDense` **passes** 略降；**mask** 下 **`_ensemblePhase` 去掉 sin 微抖** 使全队主拍更恒定；**crisp** 去相关幅略收。研究笔记 **`docs/research-pretext-typography.md` §5** 记性能与 Pretext 式取舍。 |
