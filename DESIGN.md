...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.34.0**：**入门说明**：页眉 **?** 打开原生 `<dialog>`，与 §2～§2.5 叙述对齐。**矛盾清理**：`§2.2`/`§2.3` 回稿手势与 `app.js` 实现一致。**UI**：主容器 `max-width` 放宽至 **940px**；侧栏 **`:focus-visible`** 焦点环；**`prefers-reduced-motion`** 下弱化按钮缩放与 `hint`/`toast`/`glyph` 过渡；诗笺脚注 **URL** 示例修正（`form=mega&macroText=` 等）。**「动」关→开**：短时 `_glyphFlash` 确认（mask 路径仍压低）。**`AGENTS.md`/`README` 首段**：去除「宣纸主视觉 / AI 面板」与当前产品不一致的表述。 |
