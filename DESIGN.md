...
- **形态**：多种剪影（含动物、几何如李萨如/旋迹等）；**新增**：低阶**傅里叶闭合轮廓**（类平滑运动外形）、**蔷薇线**与 **Gerono 双纽线**抽象填充；**巨字**形态用栅格采样大字笔画让小字粒子铺满（字元来自 `?mega=` 或文稿首字）；**时分秒**点阵（`chrono`）每秒刷新；龙为**西方四足飞龙**；无「月」形。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.3.1**：`isLayoutLockedForm` 统一冻结几何形（李萨如/旋迹/傅里叶/蔷薇/双纽/巨字/数字/计时/文稿）的游走、波纹、内部抖动与赛璐璐叠层；曲线目标改为**顺序沿轨**采样并 `ordered:true`；跳过「一格一字」螺旋挤占以免曲线崩坏；默认 `fluidStrength`↓、`gridMarchSpeed`↓、步进预算↓；浅色格线对锁定形态隐藏；整像素字重绘。 |
| 先前 | **3.3.0**：傅里叶/蔷薇/双纽/巨字/chrono 等。 |
