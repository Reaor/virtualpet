...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.14.0**：**巨字字腔**：笔画壳层改为 **仅自画布外背景** 播种距离场（封闭字谷白不参与），并 **剔除四邻贴字腔的 ink 像素**，避免「口、目、圆」内被小字填满；默认壳深 **3**；剪影用 **无衬线字栈**（Noto Sans SC / 思源黑体等）+ **600** 字重以利字腔。**计时**：修复 `setForm` 将 `_clockMinuteSlot`/`_chronoSecondSlot` 置 **-1** 导致与当前时间永不同步、**无法按分/秒刷新**的问题；改为写入 **当前槽位**；`pet` 模式下 **非 idle**（如觅食）亦刷新 `clock`/`chrono`。 |
