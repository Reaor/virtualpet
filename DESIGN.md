...
1b. **先静后动（弱垫底链）**：关「体内动」且开「辨」、未开「廓」时，以 **极低 α 的整张 mask 图** 叠在字粒之下，作为「先见形体」的弱锚；开「廓」则改用 **显式垫底强度**；开「体内动」时 **不叠整张 mask**（避免双轮廓与合成开销）。  
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.33.6**：**呈现关「动」= 真静形**：`presSleepLock` 将 mask 剪影字粒 **每帧锁到壳投影可走格**（跳过曼哈顿追赶）；**关** `strictSilGrid` 谐扰与 **蛇行格**、**亚格颤绘移**、**空位拉回**（`_stepSilhouetteVacancyInpull`）；生命周期 **α 步幅略收**。**叠分**：呈现剪影 **拖曳中仍执行**；遍数 **12→18**；邻格优先序 **确定性旋转**。**「动」偏好**：`snapshot` 仅写入 **呈现**套；`apply` 在待机层从 **`_arcPrefs.presentation`** 读体内动开关，避免待机桶 `presentationGlyphDynamics:true` 误伤；待机侧点「动」立即 **`presentationGlyphDynamics` 对齐**。**文档**：`docs/typography-forward.md`（排版难因、可借技术、迭代顺序）。 |
