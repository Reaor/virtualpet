...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.12.0**：**贴墙「卡住」**：锚点夹紧改用与碰撞一致的 **整身半径**（旧版 `0.42×r` 会把目标点拉到墙内带，与 `PB.resolve` 冲突）；撞墙 **切向滑移冲量** + 更强 `_wallShatter`；**漂浮轮廓**：字灵/文稿模式 **不画扩张涟漪线圈**（仅 intro 保留），**虚线活动区**默认隐藏、`?dev=1` 显示。**巨字**：`gridCell` 联动 spread/enforce、更强 `enforceTargetsMinSpacing`、略降粒子上限。**速** 按钮修复（不再误触睡眠）；`gms` 作用于 **巡逻、整体弹簧、流体弹簧、华容道间隔**。 |
