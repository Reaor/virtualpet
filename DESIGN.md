...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.15.0**：**巨字**：粒子数按 **填充面积×(1−留白率)/格面积** 估算，并与外轮廓带下限对齐，上限 **280**，格内预留空位。**边缘/内部** 分层：`edge` 高为 **外环**（小 `wanderRad`、少滑步），`edge` 低为 **深内部**（大 `wanderRad`、`_pickWanderDelta` 多尝试）。新增 **`_tryMegaSlideIntoVoid`**：内部字优先 **滑入相邻空 walkable 格**（同步 `tx/ty/mgx`），与加权 **华容道互换** 并存；略 **压低** 巨字 crisp/流体微摆，整体轮廓更稳、动态主要来自格上移让。 |
