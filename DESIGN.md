...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.6**：**侧栏点击修复**：`app.js` 在 **`window` `mouseup`** 上曾**无条件** `preventDefault()`，按 DOM 约定会**取消同一指针序列上的 `click`**，导致左侧 `.rail-tool` 等仅依赖 `click` 的控件**看似失效**；改为仅在**画布指针会话**（`downPos` / 拖拽相位 / `dragging` / 回稿蓄松）活跃时默认阻止。`styles.css`：`playfield` **`isolation: isolate`**、舞台 **`z-index: 0`**、左右栏 **`align-items: stretch`** 与 **`pointer-events: auto`** 作叠放与命中加固。 |
