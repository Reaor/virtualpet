...
### 2.5 呈现巨字 · 和谐场、统一速率与淡出重生（一脉相承）

> **问题意识**：若巨字内部仍靠「各字乱跑、华容道、随机波」维持动感，则难以同时满足 **高辨识度轮廓** 与 **可控参数**；易出现速率不均、闪烁感，且与「速」钮的心理模型不一致。  
> **统一原则**（后覆盖前）：**呈现层 + `mega`** 以 **可读剪影为第一目标**；动感改为 **全场共享的慢谐波 + 静态空间相位**（随机性来自布局与相位差，不来自多频乱叠）；**速** 钮统一缩放该谐波的时间尺度。  
> **技术取向（淡出重生）**：粒子若落在剪影 **mask 不可走** 的格上（视为离轮廓/破坏形态），则 **alpha 淡出至消失**，再在笔画附近 **合法格重生**，形成「生灭」循环，替代强行内部乱流。  
> **待机层 / 其他形态**：不强制本方案；仍可用原有游走与华容道（与呈现层巨字区分）。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.19.0**：**呈现巨字革新**：`presentation`+`mega` 启用 **和谐场**——关闭游走/华容道/巨字滑步；体内位移改为 **单一慢谐波 × `glyphMotionSpeed`**（去 per-glyph 多频与 `patrolAmpMul` 差）；流体/抖散/撞墙溃散 **大幅减弱**。**淡出重生**：格心不可走 mask 时 **alpha 衰减至 0**，再在锚点邻域 **合法格重生**。策划见 **2.5 节**。 |
