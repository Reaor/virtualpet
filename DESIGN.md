...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.18**：**贴边交互**从「撞散/脉冲物理感」改为 **橡皮泥式挤压**：`_tickDragWallPhysics` **不再调用 `_wallShatter`**，改存贴边法向 + 格迈前 **平滑切向摊开 + 微鼓入场**；**松离贴边**后仍靠 `_wallRegroupK` 缓聚（贴边时**不**与「拉回家格」对冲）。**非拖**碰壁去掉 `_wallShatter`，仅轻光晕 + 切向滑速。沙拨巨字手指贴边不再 `_wallShatter`，只抬光晕。**3.35.16–17** 绘移冲量与构建号节奏保留。 |
