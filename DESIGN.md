...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.10.1**：**移除** immutable 形态在画布上 **绘制 mask 剪影**（仅保留离屏栅格 walk，不再「透出」大字水印）；**减轻叠字**：`spreadTargets2D` 推力加大、巨字 **spreadMin/格距/粒子上限** 调整、`_separateOverlappingGridGlyphs` 对 mask 形态 **双遍** + **更大螺旋搜索半径**。 |
| 先前 | **3.10.0**：**目录**：`docs/ZILING_LAYOUT.txt` + `js/ziling/play-bounds.js`（画布边界/反弹）；计时/时分秒 **mask + 格距对齐**、巨字 **shell 加粗**、immutable **统一相位** 体内微动；去掉地面 **色团径向渐变** 与 **绘制层 lag 偏移**；**边界** 全幅 + **撞边反弹** + **碎散涟漪**（3.10.0 曾误加 **淡显轮廓**，本版撤销）。 |
