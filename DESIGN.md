...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.11.0**：**撞墙**：`resolve` **贴边即反弹 + kick**（零速度也有反馈）；**拖拽顶虚线框**溃散；画布 **虚线活动区**；**涟漪**限速/限半径/淡出/条数上限（减轻漂浮线圈）；**巨字** `enforceTargetsMinSpacing` + 更保守粒子 + 更强 spread；侧栏 **速** 调 `glyphMotionSpeed`；**标题**「开发者页控制台」。 |
