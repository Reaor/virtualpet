...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.22.0**：**亚格绘制位移** `_silDrawOx/_silDrawOy`（平滑跟随谐波目标），解决「格点四舍五入后大量字像素静止」。**剪影谐波** 加 **弱倍频 + 索引相移** 使字字有微动且同拍。**`glyphMotionSpeed`** 显式驱动 `ensBoost`、`speedVis`、`marchGms`。**叠分** mask 4 遍 + kao `rMax`。**剪影** 关 `flashBoost`、缓和 lifecycle alpha。 |
