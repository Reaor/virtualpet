...
- **觅食 / 换形 / 快捷形态**：左栏（**层**·变·觅·墨·颜·浮·眠·抖）。**层** 切换 **待机 / 呈现** 两套层级：待机栏仅数学曲线与软团；呈现栏为计时、颜文字与巨字输入行；**色 / 墨 / 浮** 在两层间 **分套记忆**（体内节拍与沿格追赶为全模态统一常量，见 `pet.js` 的 `FIXED_GLYPH_MOTION_SPEED` / `FIXED_GRID_MARCH_SPEED`）。「变」仅在 **当前层级** 的形态序列内循环。
...
> **问题意识**：仅靠「各字独立频率的游离、华容道、强流体」维持动感时，**剪影轮廓**与**可读性**常被内部乱流破坏；多频叠加还会导致 **速率不均、闪烁感**；因此体内节拍与沿格追赶改为 **统一低速常量**，便于看清轨迹。
...
2. **统一节拍**：体内位移以 **全队共享相位** `_ensemblePhase` 为主驱动；与层级 `timeScale` 经 **`motionTimeBlend`** 组合，避免「有的字快、有的字慢」的失控对比；**节拍基线**为固定低速（不再提供侧栏「速」挡位）。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.22**：**匀速纵横 + 减负**：曼哈顿迈格改为 **`_gridMarchFrameAcc` + `GRID_MARCH_CELLS_PER_SEC`**（与 `marchGms` 解耦），全字同帧共享 `stepBudget`；**格移开时禁用格间 ease**（免格心对角飘移）。**换形**：叠分遍数封顶、**不换蛇道**、剪影**跳过第二遍叠分**；`FIXED_GLYPH_MOTION_SPEED` **0.5**。 |
...
| 先前 | **3.35.9**：**呈现剪影辨形 + 侧栏命中**：巨字/颜 **呈现剪影**（`megaSilPres`）下 **统一逻辑字号缓存** `_presSilUniformBodyPx`、**每帧共用整数 `px`**、**缘 alpha / 浮光 / 闪光 / 墨色场与 cel 位置脉动** 在关内动时拉平；**叠分** 在 **关内动**（`presGlyphSleep`）时恢复 **每帧第二遍**；**规整** 后 **双遍疏散**。`app.js`：**左栏 `click` 委托**到 `.rail.rail-left` + `closest('.rail-tool[data-action]')`，与 **mouseup 仅在 `petHost` 内 preventDefault** 组合，减轻「层」等 **click** 丢失。 |
| 先前 | **3.35.8**：**侧栏可读 + 顺滑/有序**：左栏 **188px**、分区 **标题+一句说明**、主要按钮 **双行（主字+副标）**；呈现 mask 剪影 **叠分第二遍隔帧**（关内动不再每帧双遍）、`presDense` **passes** 略降；**mask** 下 **`_ensemblePhase` 去掉 sin 微抖** 使全队主拍更恒定；**crisp** 去相关幅略收。研究笔记 **`docs/research-pretext-typography.md` §5** 记性能与 Pretext 式取舍。 |
| 先前 | **3.35.7**：**字体与研究文档**：`:root` 引入 **`--font-app`**（LXGW 优先 + 思源/系统 UI 殿后），`--font-ui` / `--font-serif` 同指以 **统一同页 DOM 字体栈**；等宽独立 **`--font-mono`**。巨字离屏 `FONT_MACRO_CJK_OPEN` **前置 LXGW**，与画布/DOM 量宽口径更一致。新增 **`docs/research-pretext-typography.md`**（Pretext 原则 ↔ 代码锚点、§2.5 运动索引、GitHub Compare 自检）；`HANDOFF` 增 **PR/Compare** 指引；`typography-forward.md` 链到姊妹篇。觅食飘字读 **`--font-app`**。 |
| 先前 | **3.35.6**：**侧栏点击修复**：`app.js` 在 **`window` `mouseup`** 上曾**无条件** `preventDefault()`，按 DOM 约定会**取消同一指针序列上的 `click`**，导致左侧 `.rail-tool` 等仅依赖 `click` 的控件**看似失效**；改为仅在**画布指针会话**（`downPos` / 拖拽相位 / `dragging` / 回稿蓄松）活跃时默认阻止。`styles.css`：`playfield` **`isolation: isolate`**、舞台 **`z-index: 0`**、左右栏 **`align-items: stretch`** 与 **`pointer-events: auto`** 作叠放与命中加固。 |
| 先前 | **3.35.5**：**呈现剪影字形与谐步微优化**（对齐 `§2.5`）：严格谐步格 **Schmitt 滞回**（`g._silHistDgx` / `_silHistDgy`）+ 相位空间微抖，减轻 ±1 格阈值附近抖振；侧栏「颤」下 **亚格绘移** `_silDrawOx/_silDrawOy` 按 **残差相对 cap** 自适应 **exp 收敛率**（大偏移跟得快、近目标柔）；`crispMotion` 与 **流体 crisp** 支路对 **格位哈希弱去相关**（约 ±5% 幅，仍守 `_ensemblePhase` 主拍）；**呈现巨字单层** `megaSilPres` **缘/心 alpha 对比略拉** 以利辨形。 |
| 先前 | **3.34.6**：**主支线合并**：在 **3.34.5**（`_dragShellWorld` / `_bodyWorldForShell`、拖曳中叠分、单字壳匀、关内动下关 `strictSilGrid` 谐扰、华容 swap **α×0.78**、`mergePresentation… timeScale×0.32` 等）之上，**关「内动」** 时 **`presSleepLock`** 每帧将 mask 剪影字粒 **锁到壳投影可走格**（停曼哈顿追赶、蛇行格占用、亚格颤绘移、空位拉回）；**`presentationGlyphDynamics`** 仅 **`snapshot` 入呈现套**；待机 **`apply`** 从 **呈现套** 读取，避免待机桶误伤；**`_separateOverlappingGridGlyphs`** 在 **呈现剪影拖曳** 或 **拖曳解耦** 时仍执行（`presDense` 或 `decoupDrag`）；生命周期 **α maxStep** 关内动 **0.48**。**文档**：`docs/typography-forward.md`。 |
| 先前 | **3.34.5**：**拖曳与整体解耦（起步）**：呈现 mask 剪影且 **`gridMarch`+`gridSnapping`** 时 **`_dragShellWorld`** 为跟手剪影中心，**`pos` 拖曳中冻结**（不积分 `vel`），**`endDrag`** 才把锚点对齐；**`_bodyWorldForShell`** 统一格迈 / 叠分 / 华容道 / 命中半径。**叠分**：拖曳解耦时 **仍执行** `_separateOverlappingGridGlyphs`；呈现层遍数 **18→20**（拖曳时）；邻格搜索 **确定性旋转** 替代随机洗牌。**单字巨字壳**：`buildFormLayoutData("mega")` 在呈现且 **单 grapheme** 时 **略增 spread/enforce 与遍数**。**规整关内动**：**关谐步离散扰动**（`strictSilGrid && !(presSilHarm && presGlyphSleep)`）；**亚格颤** lerp 略柔；生命周期 **α 步幅略收**；华容道 swap **α 乘 0.78**（轻淡）。**呈现关内动 `timeScale`**：**×0.32**（较 0.22 略抬），使侧栏 **「速」** 对整体节拍更可感。**3.34.4** 条目（规整先静、轻淡换位等）仍有效。 |
| 先前 | **3.34.4**：**规整**：`applyPresentationSilhouetteHarmonicCalm` 将 **`_huarongNextAt` 推迟约 900ms**，轮廓先稳再启邻格互换；**关内动**（`presSleep`）下华容道 **swapPair** 成功后对两字粒 **α 一次压低**（淡隐再接生命周期淡入）。**拖曳**：呈现 mask 剪影且 **`gridMarch`** 时 **`_dragTargetPos`** 存指针目标，**`pos` 每帧 lerp 靠近**，松手对齐；格向残差仍按目标位移累加。**3.34.3** 条目（拼满/逐字排布、颤幅、华容道在剪影等）保持有效。 |
| 先前 | **3.34.3**：**呈现巨字排布**：`_arcPrefs.presentation.presentationMegaLayoutMode` **`fit_canvas` | `sequential_chars`**；侧栏 **「排布」**（`cyclePresentationMegaLayoutMode`）。**拼满**：`macroTextBalancedWrapShort` 短串分行 + `resolveMegaLayoutInput` 在呈现下按估算粒子数 **迭代略缩 `megaLayoutScale` 等效**；`computePresentationMegaGridCell` 按画布与字数收紧 **`gridCell`**。**逐字**：`_megaSeqIdx` + `_tickPresentationMegaAux` 约 **2.35s** 换一字并 `setForm`。**颤幅**：`silhouetteJitterAmpMul` + **「颤幅」** 键，亚格绘移 **`cap` 乘子**。**拖曳**：`_dragResidualLx/y` + 每字异相偏移，减轻整块剪影平移感。**华容道**：呈现 mask 剪影路径 **不再跳过** `_tryHuarongAdjacentSwaps`；**关内动** 时冷却 **×2.55**，**开内动** **×0.88**。 |
