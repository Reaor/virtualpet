...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.5**：**呈现剪影字形与谐步微优化**（对齐 `§2.5`）：严格谐步格 **Schmitt 滞回**（`g._silHistDgx` / `_silHistDgy`）+ 相位空间微抖，减轻 ±1 格阈值附近抖振；侧栏「颤」下 **亚格绘移** `_silDrawOx/_silDrawOy` 按 **残差相对 cap** 自适应 **exp 收敛率**（大偏移跟得快、近目标柔）；`crispMotion` 与 **流体 crisp** 支路对 **格位哈希弱去相关**（约 ±5% 幅，仍守 `_ensemblePhase` 主拍）；**呈现巨字单层** `megaSilPres` **缘/心 alpha 对比略拉** 以利辨形。 |
