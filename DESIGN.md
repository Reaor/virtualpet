...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.13.0**：**巨字**：壳层像素 **分层抽样**（避免扫描序断墨）；spread/enforce 后 **远点拉回壳层** + 再间距整形；**字数/壳面积** 联合估算粒子上限 **240**；多字串 **自动缩小字号**。**计时/时分秒**：点阵目标 **按亮格均分**（n≤格数时跨格铺开，避免半幅缺字）；**点块略加粗**；切形时按 **亮格数×密度** 自动调整粒子数；**格距** 略增。**巨字** immutable 体内波 **振幅+42%** 以利「形内单字微动」。 |
