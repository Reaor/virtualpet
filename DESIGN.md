...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.9**：**呈现剪影辨形 + 侧栏命中**：巨字/颜 **呈现剪影**（`megaSilPres`）下 **统一逻辑字号缓存** `_presSilUniformBodyPx`、**每帧共用整数 `px`**、**缘 alpha / 浮光 / 闪光 / 墨色场与 cel 位置脉动** 在关内动时拉平；**叠分** 在 **关内动**（`presGlyphSleep`）时恢复 **每帧第二遍**；**规整** 后 **双遍疏散**。`app.js`：**左栏 `click` 委托**到 `.rail.rail-left` + `closest('.rail-tool[data-action]')`，与 **mouseup 仅在 `petHost` 内 preventDefault** 组合，减轻「层」等 **click** 丢失。 |
