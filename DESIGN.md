...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.34.6**：**主支线合并**：在 **3.34.5**（`_dragShellWorld` / `_bodyWorldForShell`、拖曳中叠分、单字壳匀、关内动下关 `strictSilGrid` 谐扰、华容 swap **α×0.78**、`mergePresentation… timeScale×0.32` 等）之上，**关「内动」** 时 **`presSleepLock`** 每帧将 mask 剪影字粒 **锁到壳投影可走格**（停曼哈顿追赶、蛇行格占用、亚格颤绘移、空位拉回）；**`presentationGlyphDynamics`** 仅 **`snapshot` 入呈现套**；待机 **`apply`** 从 **呈现套** 读取，避免待机桶误伤；**`_separateOverlappingGridGlyphs`** 在 **呈现剪影拖曳** 或 **拖曳解耦** 时仍执行（`presDense` 或 `decoupDrag`）；生命周期 **α maxStep** 关内动 **0.48**。**文档**：`docs/typography-forward.md`。 |
