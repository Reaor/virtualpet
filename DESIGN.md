...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.10.0**：**目录**：`docs/ZILING_LAYOUT.txt` + `js/ziling/play-bounds.js`（画布边界/反弹）；**辨识度**：计时/时分秒 **mask + 格距对齐**、巨字 **shell 加粗**、清晰形态下 **淡轮廓底** + **统一相位** 体内微动；**拖拽**：去掉地面 **色团径向渐变** 与 **绘制层 lag 偏移**（避免「色团与字灵分离」）；**边界**：`ZiLingPlayBounds` 全幅活动区 + **撞边反弹** + **碎散涟漪**。 |
