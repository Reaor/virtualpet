...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.35.19**：贴边挤压 **丝滑化**：法向 **指数 lerp**（`_dragWallPinNxSm/y`）、`_wallRegroupK` **smoothstep** 驱动幅面、切向用 **`tanh` 限幅**；贴边时 **压低 rumble**；外框光晕 **smoothstep + 略弱线宽/透明度**；离边 **缓收** `_dragWallPinNxSm`。**3.35.18** 橡皮泥语义与合并主干不变。 |
