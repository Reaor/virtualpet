...
- **觅食 / 换形 / 快捷形态**：左栏（变·觅·眠·抖 + **按类别分组的形态按钮**：软象、动物、纹·时、曲线·巨字、**颜文字**、数字等）；**呈·灵·空·回**在右栏与文稿强相关。
...
- **形态**：多种剪影（动物类暂缓大改）；几何含李萨如/旋迹、**傅里叶轮廓**、**蔷薇线**、**双纽线**；**巨字**按**整串** `macroText`（页面输入或 `?macroText=` / 兼容 `?mega=`）或文稿首字做剪影采样；**时分秒**点阵（`chrono`）；**颜文字**（`kao_*`，小字沿 `(´∀`)` 等轮廓）；软象仅保留 **软团**（撤下云/心/星等近似形）；撤下旧「汉字拼圆脸」表情系列。龙为**西方四足飞龙**。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.4.0**：`_resize` 每帧更新 **`center = (width/2, height/2)`** 并在 `script`/`pet` 下把 **`pos`/`anchor` 拉回中心**（修复整体偏左上）；软象仅 **团**；**巨字**支持多字串 + 面板「化身字形」输入；**颜文字** `kao_*` 替换旧 `emoji_face_*`；烦躁换形备选移除已删形态。 |
| 先前 | **3.3.3**：**设备像素对齐** `_snapLogicalToDevice`；浅色主题躯体 **关闭 cel 渐变与四向描边**（单层实色）；暗色 cel **缩描边**；`gridUnity` 小角度字不旋转；字重 600、最小字号抬高；朱砂浅色去 shadow。 |
