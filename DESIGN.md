...
- **觅食 / 换形 / 快捷形态**：左栏（变·觅·眠·抖 + **按类别分组的形态按钮**：软象「团·须·卷·母」、动物、纹·时、曲线·巨字、**颜文字（九种）**、数字等）；**呈·灵·空·回**在右栏与文稿强相关。
...
- **形态**：多种剪影（动物类暂缓大改）；几何含李萨如/旋迹、**傅里叶轮廓**、**蔷薇线**、**双纽线**；**巨字**按**整串** `macroText`（页面输入或 `?macroText=` / 兼容 `?mega=`）或文稿首字做剪影采样；**时分秒**点阵（`chrono`）；**颜文字**多种 `kao_*`（小字沿颜文字轮廓，`isLayoutLockedForm` 按前缀锁定）；**软象**含 **软团** + **触须团**（径向 cos₊^p + 轮廓 mask）、**卷须**（边界 ripple）、**水母**（钟形伞 + 贝塞尔拖须采样）。龙为**西方四足飞龙**。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.5.0**：**软象**增加径向 **触须团**（cos₊^p 星形 + mask）、**卷须**（边界 ripple 调制）、**水母**（钟形 + 贝塞尔拖须栅格采样）；**颜文字**扩展至 9 种（`kao_party`…`kao_shrug`），`isLayoutLockedForm` 对 `kao_*` 前缀统一锁定。 |
| 先前 | **3.4.0**：`_resize` 每帧更新 **`center`** 并在 `script`/`pet` 下 **重锚中心**；软象仅 **团**；**巨字** `macroText`；**颜文字** 替代旧表情。 |
