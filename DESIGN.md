...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.9.0**：侧栏 **加宽**（约 108px）+ 待机按钮 **3 列**；**拖拽**时格点目标统一用 `pos`（修复「部分字不跟手」），拖曳滞后仅作 **绘制层** 位移；**拖曳中**跳过叠字分离；**华容道**邻格随机互换（非 script/计时/锁运动形态）；**墨/浮**对比度加大；巨字粒子 **slot** 略增、文稿 mask 行距与布局 **1.42** 对齐。 |
