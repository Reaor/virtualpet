...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | **3.34.4**：**规整**：`applyPresentationSilhouetteHarmonicCalm` 将 **`_huarongNextAt` 推迟约 900ms**，轮廓先稳再启邻格互换；**关内动**（`presSleep`）下华容道 **swapPair** 成功后对两字粒 **α 一次压低**（淡隐再接生命周期淡入）。**拖曳**：呈现 mask 剪影且 **`gridMarch`** 时 **`_dragTargetPos`** 存指针目标，**`pos` 每帧 lerp 靠近**，松手对齐；格向残差仍按目标位移累加。**3.34.3** 条目（拼满/逐字排布、颤幅、华容道在剪影等）保持有效。 |
