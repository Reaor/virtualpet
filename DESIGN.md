...
- **觅食 / 换形 / 快捷形态**：左栏（变·觅·眠·抖 + 团·猫·龙·钟·线）；**呈·灵·空·回**在右栏与文稿强相关。
- **字序与密度**：粒子数随**输入非空白字数**增长（有上下限）；非眉眼粒子**严格按文稿顺序循环赋字**，化灵与拖回文稿使用**同一份 `scriptLines`**，保证「这一段话」一致。
- **文稿清晰**：`script` 形态关闭赛璐璐描边叠层、减弱呼吸与液体波，字重与字号略抬升，避免糊成一团。
...
| 日期（会话） | 决策 |
|--------------|------|
| 最新 | 侧栏控件扩大画布；`script` 清晰渲染；粒子数∝输入字数；躯体字序与文稿一致；修复 `mT` 语法错误（3.2.4）。 |
