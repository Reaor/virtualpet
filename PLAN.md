...
> **当前构建**：见 `index.html` 中 `ziling-build`（与页头 `buildStamp` 一致；近期为 **3.33.2** 起）。  
> **接手说明**：`HANDOFF.md`（架构、常见坑、需求归纳；**非**完整对话逐字存档）。  
...
| 运动美感、凑成形体的紧凑感 | **部分完成（3.33.2）** | **3.33.2**：修正 **待机层被呈现专用逻辑误伤**（`contourStatic` / 流体 ×0.1 / 生命周期淡出 / `fillCount` 截断）；见 `HANDOFF.md`。继承 3.33.0 巨字 UI 与 3.32.4 叠分/性能。**未做**：B-spline 格路径、显式速度场、GPU 粒子管等 |
