...
> **当前构建**：见 `index.html` 中 `ziling-build`（与页头 `buildStamp` 一致；近期为 **3.32.4** 起）。  
...
| 运动美感、凑成形体的紧凑感 | **部分完成（3.32.4）** | 已 **降低** `fluidStrength` 默认、**减小**巡逻 `pAmp`；**3.32.3～4**：呈现剪影 **每帧一格**、正交邻叠分、生命周期淡入淡出；**3.32.4**：**`fillCount` 估格上限截断粒子**（防结构性共格）、**开「动」不绘制整张巨字 mask 垫底**、**减重复叠分与背景格线**、mask 离屏 **`willReadFrequently:false`**。**未做**：B-spline 格路径、显式速度场、GPU 粒子管等 |
