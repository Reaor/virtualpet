...
> **当前构建**：见 `index.html` 中 `ziling-build`（与页头 `buildStamp` 一致；近期为 **3.33.1** 起）。  
...
| 运动美感、凑成形体的紧凑感 | **部分完成（3.33.0）** | 继承 **3.32.4** 叠分/粒子上限/性能；**3.33.0**：**整块巨字灰轮廓** 仅 **「廓」开** 时绘制（辨不再隐式画垫底）；**巨字身幅** `megaLayoutScale`（与 `gridCell` 下限取 max）+ **容纳** `macroFitMode`（缩字/截断/双行）；侧栏 **双列分区**。**未做**：B-spline 格路径、显式速度场、GPU 粒子管等 |
