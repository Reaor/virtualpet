...
> **当前构建**：见 `index.html` 中 `ziling-build`（与页头 `buildStamp` 一致；近期为 **3.33.3** 起）。  
...
| 运动美感、凑成形体的紧凑感 | **部分完成（3.33.3）** | **3.33.3**：侧栏 **四区块** + 各键 **`title` 长说明**；底部 **control-block** 分区标题；呈现关「动」时 **mask 垫底略增**、**生命周期淡入/淡出略加强**；`HANDOFF` 增 UI 映射。**未做**：B-spline 格路径、GPU 粒子等 |
