...
- **`DESIGN.md`** — 产品宪法：**辨形优先**、**形场**抽象、壳/芯/边界、**外部矩阵/API 预留契约**（原则级，暂不实现 AI）。  
- **`PLAN.md`** — 工程与阶段计划：形/动/叙事三层、近中远期里程碑、风险与验收口径。

新功能或改运动轨前，建议先对照上述两文档，避免与长期形场接口服务冲突。
...
├─ js/ziling/shape-field.js   形场（可走格 + 拓扑壳层，辨形 / 矩阵伏笔）
├─ pet.js         字灵本体（粒子物理、形态、渲染引擎）
├─ app.js         应用层（交互、喂食、UI 集成）
├─ DESIGN.md      产品设计书（意图、辨形、形场与矩阵预留）
└─ PLAN.md        计划书（阶段路线、风险、验收）
```
