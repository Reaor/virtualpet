...
- **近期版本在「叠字 / 闪现 / 贴边体感」与「每帧成本」之间做过多轮平衡**（见 `IMPROVEMENTS` 3.35.22 起；**3.35.27** 针对巨字呈现再压叠分/格迈峰值）。若用户反馈 **卡**，优先 **量**（`?dev=1` 已有分段时间）再 **减频或加条件**，而不是先讨论换框架。
...
- **`_gridMarchFrameAcc` + `GRID_MARCH_CELLS_PER_SEC`**：全字共享 `stepBudget`；呈现剪影与待机 **同一套分数扣减**（3.35.25）；**3.35.27** 起呈现 mask 巨字/颜 **单独 cap 每帧 1～2 格** 并 **`presMarchAccMul`**，避免与待机同用 3 格/帧时叠分+march 双峰值。
